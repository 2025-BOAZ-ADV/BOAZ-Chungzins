#!/usr/bin/env bash
# 실행 전 다음 구문을 bash에 입력(최초 한번만 하면 됨): chmod +x ./testing/experiments/run_sweep.sh
...
mkdir -p testing/experiments/runs
MASTER_LOG="testing/experiments/runs/sweep_$(TZ=Asia/Seoul date +%Y%m%d_%H%M%S).log"
